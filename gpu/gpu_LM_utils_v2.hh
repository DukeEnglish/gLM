...
std::pair<bool, std::string> testQueryNgrams(LM& lm, unsigned char * btree_trie_gpu, unsigned int * gpu_first_lvl, StringType arpafile) {
    ...
    searchWrapper(btree_trie_gpu, gpu_first_lvl, gpuKeys, num_keys, results, lm.metadata.btree_node_size, lm.metadata.max_ngram_order);
...
inline std::vector<std::string> interactiveRead(LM &lm, unsigned char * btree_trie_gpu, unsigned int * gpu_first_lvl, bool addBeginEndMarkers = true) {
    std::string response;
